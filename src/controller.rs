...
impl Controller {
    ...
    pub fn compute_asset_allocation(&self, timestamp: TimeStamp) -> GenResult<AssetAllocations> {
        ...
        let asset_scores: Vec<AssetScore> = assets
            .iter()
            .flat_map(|a| {
                compiled_strategy.asset_score(a.clone(), timestamp, self.data_client.new())
            })
            .collect();
...
mod tests {
    ...
    fn foo() -> GenResult<()> {
        ...
        // let result = controller.compute_asset_allocation(start)?;
        // assert_eq!(result.timestamp, start);
